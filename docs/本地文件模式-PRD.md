...
> **⚠️ 本方案已废弃，请参见 [`docs/文件上传生成用例-PRD.md`](./文件上传生成用例-PRD.md)**
>
> **变更概要**：输入改为仅上传入口（不再目录扫描）；导出改为仅 Excel 下载（不配置导出路径）；文件类型改为 .md + .xlsx 多文件；产出改为**仅新用例表**（不再产出理解内容/问题点）。本 PRD 已废弃，以 `docs/文件上传生成用例-PRD.md` 为准。

---

<details>
<summary>以下为原方案内容（仅供历史参考）</summary>
